"""Milestone 10: Customer Analytical Evidence Tool - Deterministic tests."""
...
try:
    ...
    print(f"[smoke M10] FAIL - import: {e}", file=sys.stderr); sys.exit(1)
...
def _check(r, n):
    ...
    return f
async def main():
    if not os.getenv("GEMINI_API_KEY"): print("[smoke M10] SKIP"); sys.exit(0)
    failures=[]
    print("[smoke M10] CHECK 1-3: MCP Toolbox starts with 11 tools...")
    ...
    if missing: failures.append(f"Missing: {missing}"); print(f"[smoke M10] FAIL - {missing}",file=sys.stderr)
    else: print(f"[smoke M10] PASS - {len(found)} tools")
    if "get_customer_behavior_profile" not in found: failures.append("customer tool missing")
    else: print("[smoke M10] PASS - customer tool exists")
    print("[smoke M10] CHECK 4-5: Customer tool queries BigQuery...")
    try:
        t=next(x for x in tools if x.name=="get_customer_behavior_profile")
        r=await t.run_async(args={"start_date":"2026-08-01","end_date":"2026-08-28"},tool_context=None)
        f=_check(r,"get_customer_behavior_profile")
        if f: failures.extend(f); print(f"[smoke M10] FAIL - {f[0]}",file=sys.stderr)
        else:
            content=r.get("content",[])
            print(f"[smoke M10] PASS - returned {len(content)} rows")
            data=[]
            for item in content:
                if isinstance(item,dict) and "text" in item:
                    try: data.append(json.loads(item["text"]))
                    except: pass
            print("[smoke M10] CHECK 6-8: Data validity...")
            for c in data:
                if c.get("total_spend") is None or c.get("total_spend",0)<0: failures.append(f"Invalid spend: {c.get('customer_id')}")
                if c.get("bill_count",0)<1: failures.append(f"Invalid bill_count: {c.get('customer_id')}")
                if not c.get("customer_id"): failures.append("Missing customer_id")
            if not any("Invalid" in str(x) or "Missing" in str(x) for x in failures): print(f"[smoke M10] PASS - {len(data)} customers, all valid")
    except Exception as exc:
        failures.append(f"Tool failed: {exc}"); print(f"[smoke M10] FAIL - {exc}",file=sys.stderr)
    print("[smoke M10] CHECK 9-10: Agents import, no BQ...")
    ...
    if not any("BQ" in f or "missing toolbox" in f for f in failures): print("[smoke M10] PASS - agents OK")
    print(f"\n{'='*60}")
    if failures:
        print(f"[smoke M10] RESULT: FAIL - {len(failures)}"); [print(f"  - {f}") for f in failures]; sys.exit(1)
    else: print("[smoke M10] RESULT: PASS"); sys.exit(0)
